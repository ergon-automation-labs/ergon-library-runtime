defmodule BotArmyLibraryRuntime.NATS.Conversation.Gossip do
  @moduledoc """
  ...
  Call `Gossip.maybe_gossip/2` from your heartbeat cycle:
      ...
      BotArmyLibraryRuntime.NATS.Conversation.Gossip.maybe_gossip("gtd", %{idle: true, metric: "processed 150 events"})
  """

  require Logger

  alias BotArmyLibraryRuntime.Personality.Voice
  alias BotArmyLibraryRuntime.NATS.Conversation.Mailbox
  ...
  defp do_gossip(bot_name, opts) do
    case find_gossip_partner(bot_name) do
      ...
      partner ->
        ...
        # Always use mailbox mode (async) instead of conversation mode.
        # Conversation mode requires Manager to be available and adds complexity.
        mode = :mailbox
  ...
  defp start_gossip_conversation(from_bot, to_bot, template, message) do
    ...
    try do
      case BotArmyLibraryRuntime.NATS.Conversation.Manager.start_conversation(
             from_bot,
             to_bot,
             "gossip",
             body,
