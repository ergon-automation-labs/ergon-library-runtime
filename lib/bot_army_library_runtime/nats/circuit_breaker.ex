defmodule BotArmyLibraryRuntime.NATS.CircuitBreaker do
  @moduledoc """
  ...
  ## Usage
      ...
      case BotArmyLibraryRuntime.NATS.CircuitBreaker.allow?("my_bot:target.subject") do
        :ok ->
          # Proceed with request
          {:ok, reply} = BotArmyLibraryRuntime.NATS.Publisher.request(...)
          BotArmyLibraryRuntime.NATS.CircuitBreaker.record_success("my_bot:target.subject")
          {:ok, reply}
      ...
      {:error, :timeout} ->
        BotArmyLibraryRuntime.NATS.CircuitBreaker.record_failure("my_bot:target.subject", :timeout)
        {:error, :timeout}
  """
  ...
  defp lookup_registry(key) do
    Registry.lookup(BotArmyLibraryRuntime.NATS.CircuitBreakerRegistry, key)
  rescue
    ArgumentError -> []
  end
