defmodule BotArmyLibraryRuntime.Heartbeat do
  ...
  defp record_from_publish_opts(publish_opts, extra_opts) do
    ...
    tenant_id = Keyword.get(publish_opts, :tenant_id, BotArmyLibraryRuntime.Tenant.default_tenant_id())
    tenant_uuid = normalize_tenant_id(tenant_id)

    status =
      case Keyword.get(publish_opts, :status) do
        nil ->
          BotArmyLibraryRuntime.SynapseHealth.signal_to_status(
    ...
    uptime =
      Keyword.get(publish_opts, :uptime_seconds, BotArmyLibraryRuntime.SynapseHealth.uptime_seconds())
  ...
  defp do_persist(repo, attrs, start_mono, telemetry?) do
    ...
    try do
      result = repo.query!("SELECT current_database();")
